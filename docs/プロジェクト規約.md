...
> 下の構成が現行規約で一式入っている。config は `from comken import config`（src/config.py 不要）。
...
> （初回アクセス時にカレントの config.ini を遅延読み込み）。従来の `src/config.py`
> （`config = Config()`）は、エディタ補完スタブが欲しい場合に作ってもよい（任意）。
