...
> main.py・config.ini.example・src/run.py・src/credentials.py・使い方.md・仕様書.md・
> ERRORS.md・実行.bat・認証情報の登録.bat・.gitignore・pyproject.toml・.vscode 一式が
> 現行規約で入っている。config は `from comken import config`（src/config.py 不要）を使う。
