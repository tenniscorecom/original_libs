...
> **設定の読み方:** プロジェクトでは `src/config.py`（`config = Config()`）を使い、
> 各モジュールから `from .config import config` で参照する。`Config()` が補完スタブ
> `src/config.pyi` を自動生成するので、**VS Code + Pylance で `config.SECTION.KEY` が
> 型付き補完される**（ひな形に同梱済み。手で書く必要はない）。
> スクリプト等で手軽に済ませたい場合だけ `from comken import config`（補完なし）を使う。
