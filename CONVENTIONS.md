...
SF_CREDENTIAL_NAME: str = _cfg.get("CREDENTIALS", "SALESFORCE")
```
...
cred = load_credential(SF_CREDENTIAL_NAME)  # 事前に python -m comken.credentials で登録しておく
sf = SalesforceClient(
...
認証情報はサービス名をキーに何件でも登録できる（上書きになるのは同じサービス名のときだけ）。

```
credentials.dat（1ユーザーにつき1ファイル、中身は複数エントリ）
{
  "salesforce": { username, password, token },
  "oju_sys": { username, password, token },
  "salesforce_test": { username, password, token }
}
```

| 状況 | サービス名の付け方 | 例 |
|---|---|---|
| サイト・システムごと | システム名をローマ字で | `salesforce`, `oju_sys`（応需sys） |
| 同じサイトでアカウントを使い分ける | システム名 + 用途 | `salesforce_honban`, `salesforce_test` |

> **サービス名に使えるのは半角英数字とアンダースコアのみ。**
> それ以外（漢字・スペース・記号）はライブラリ側で弾かれる（`InvalidServiceNameError`）。

**どのサービス名を使うかは config.ini の `[CREDENTIALS]` セクションに書く。**
...
| `CredentialNotFoundError` | 認証情報が未登録 | `load_credential()` で未登録のサービス名を指定したとき |
| `InvalidServiceNameError` | サービス名が不正 | 半角英数字・アンダースコア以外を含む名前で登録しようとしたとき |
