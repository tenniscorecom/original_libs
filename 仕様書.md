...
| `runtime` | 実行モード（version / set_debug: 処理時間を DEBUG ログ / set_dry_run: 外部影響のある操作をスキップ） | なし |
...
更新手順: 開発フォルダで修正（ブランチ自由）→ main にマージ → `リリース.bat` で配布フォルダに反映
→ 各 PC は次回起動時に自動同期。

リリース.bat は反映時に `release-YYYYMMDD-HHMM` 形式の git タグを自動で打つ。
「いつの comken が配られていたか」を後から `git tag` / `git log` で追跡できる
（バージョン管理は robocopy のタイムスタンプ差分で行うため、タグは記録用）。
