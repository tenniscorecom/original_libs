...
```
共有フォルダ \\server\share\tools\comken   ← 管理者がここを更新する（配布ポイント）
...
- 持ち込みは**初回の1回だけ**: GitHub からダウンロードして共有フォルダに配置する
  （以後、外部からの持ち込みは行わない。USB 等も不可の環境）
- 以後の更新は**社内で完結**する: 社内で修正 → 共有フォルダを差し替え → 各 PC は次回起動時に自動同期
...
- この GitHub リポジトリは**社内への初回持ち込みまでの開発用**。持ち込み後の変更は社内側が正となり、
  外部とは同期しない（社内 BO 環境はオフライン）
